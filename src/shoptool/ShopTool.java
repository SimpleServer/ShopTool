...
public class ShopTool {
	...
	public static void main(String[] args) {
		...
		if (args.length == 0) {
			System.out
					.println("Usage: shoptool DIRECTORY\nThat directory should contain shop definitions\n(."
							+ extension + " extension)");
			return;
		}

		LinkedList<Shop> shops = getShops(new File(args[0]));
		String output = EventGenerator.generateShops(shops, true);
		if (output != null)
	...
	private static Shop getShopFromFile(File file) {
		...
		try {
			...
			try {
				shop.setInterestBuy(extractDouble(reader.readLine(),
						INTEREST_BUY));
			} catch (Exception e) {
				shop.setInterestBuy(DEFAULT_INTEREST_BUY);
			}
			// third line: interest sell
			try {
				shop.setInterestSell(extractDouble(reader.readLine(),
						INTEREST_SELL));
			} catch (Exception e) {
				shop.setInterestSell(DEFAULT_INTEREST_SELL);
			}
			// rest is the pricelist
			shop.setPricelist(getPriceListFromReader(reader));
		} catch (IOException ex) {
			Logger.getLogger(ShopTool.class.getName()).log(Level.SEVERE, null,
					ex);
		}
		shop.recalcPrices();
		return shop;
	...
	private static PriceList getPriceListFromReader(BufferedReader reader) {
		...
		while (true) {
			try {
				...
				Logger.getLogger(ShopTool.class.getName()).log(Level.SEVERE,
						null, ex);
			}
			if (line == null)
				break;
	...
	private static PriceListItem parseLine(String line) {
		...
		try {
			item.setMaxStock(Integer.parseInt(extract(line, MAX)));
		} catch (Exception e) {
			item.setMaxStock(DEFAULT_MAXSTOCK);
		}

		try {
			item.setNormalStock(Integer.parseInt(extract(line, NORMAL)));
		} catch (Exception e) {
			item.setNormalStock(DEFAULT_NORMALSTOCK);
		}

		try {
			item.setPriceBuy(Integer.parseInt(extract(line, PRICE)));
		} catch (Exception e) {
			item.setPriceBuy(DEFAULT_PRICE_BUY);
		}

		try {
			item.setPriceSell(Integer.parseInt(extract(line, PRICE)));
		} catch (Exception e) {
			item.setPriceSell(DEFAULT_PRICE_SELL);
		}

		try {
			item.setStockUpdateTime(Integer.parseInt(extract(line, TIME)));
		} catch (Exception e) {
			item.setStockUpdateTime(DEFAULT_TIME);
		}
